...
func GenerateJWTToken(id string) (*TokenPair, error) {
	// Fetch employee details to get role
	employeeID, _ := strconv.Atoi(id)
	employee, err := repositories.NewEmployeeRepository(bootstrap.App().DB).FindByID(uint(employeeID))
	if err != nil {
		return nil, err
	}
	...
	standardClaims := &ClaimsToken{
		Id:        id,
		Role:      employee.Role.Name, // Add role to claims
...
func GetOwnerAccessToken(c *fiber.Ctx) (*uint, error) {
	_, decrypt, _ := jwt.GCM([]byte(bootstrap.GlobalEnv.JWT.AccessToken), nil)
